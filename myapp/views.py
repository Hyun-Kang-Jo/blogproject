...
def create_pro(request):
    ...
    cafe_product.product_name = request.GET['product_name']#request에서 받은 것을
...
def show(request):#db에 있는걸 여기에 보여줘야 목록이 뜸! product부터
    products = Cafe.objects#model의 Cafe를 가져와서 쿼리셋 형태로 만들어줌. 그걸 가져온것
...
def search(request):#db에서 가져와서 일치하는 것 찾아서 띄우기. 쿼리셋 메소드 사용 all다음
    product = Cafe.objects.filter(product_name = request.GET['product_name']) #object까지가 쿼리셋 filter는 쿼리셋 메소드 해서 updateSearch하는거에서 업데이트서치이름을 가져온것.
    return render(request, 'update.html', {'product':product[0]})#이 product는 쿼리셋인데, 리스트형태임. 위의 메소드중 GET의 경우는 하나만, filter는 객체의 모든 것을 가져오지만 그래서0을 써준것

def update(request):#수정과정, but 아메리카노를 db에서 가져와서 그 정보를 에스프레소로 바꾸어주어야! 그러면 아메리카노를 가져와야함 이게 updatehtml에 59번째줄!
    product = Cafe.objects.filter(pk=request.GET['product_id'])[0] #pk는 db에있는 id 0001에 있는 그거 가져와서 아까 2가지만 넘겼으면 못쓸텐데 다 넘기고 히든해준것! productid는 히든에서 가져왔고. 그걸 가져와서 수정하는것. 굳이 보일필요 없으니 안썻던거고
